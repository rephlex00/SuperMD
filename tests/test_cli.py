...
from supermd.cli import cli

@pytest.fixture
def runner():
    return CliRunner()

@patch("supermd.cli.convert_file")
...
@patch("supermd.cli.convert_directory")
...
@patch("supermd.batches.run_batches")
def test_cli_run(mock_run, runner):
    """Test 'run' command invokes batch runner."""
    result = runner.invoke(cli, ["run", "--dry-run"])
    assert result.exit_code == 0
    mock_run.assert_called_with("config/supermd.yaml", parallelism=1, dry_run=True, debug_mode=False)

@patch("supermd.watcher.run_watcher")
...
@patch("supermd.cli.install_service")
def test_cli_service_install(mock_install, runner):
    ...
    assert result.exit_code == 0
    mock_install.assert_called_with("config/supermd.yaml", dry_run=True)

@patch("supermd.cli.start_service")
...
@patch("supermd.cli.stop_service")
...
@patch("supermd.cli.logs_service")
...
@patch("supermd.report.MetadataManager")
@patch("supermd.config.load_config")
def test_cli_meta_list(mock_load_config, mock_manager_cls, runner):
    """Test 'meta list' command."""
    from supermd.config import SuperMDConfig, JobDefinition
    mock_load_config.return_value = SuperMDConfig(
        jobs=[JobDefinition(name="Test", input="/tmp/in", output="/tmp/out")]
    )
...
@patch("supermd.converter.rebuild_metadata_directory")
@patch("supermd.config.load_config")
def test_cli_meta_rebuild(mock_load_config, mock_rebuild, runner):
...
@patch("supermd.converter.clean_metadata_directory")
@patch("supermd.config.load_config")
def test_cli_meta_rm(mock_load_config, mock_clean, runner):
    ...
    with patch("os.path.exists", return_value=True):
        result = runner.invoke(cli, ["meta", "rm"])
    assert result.exit_code == 0
