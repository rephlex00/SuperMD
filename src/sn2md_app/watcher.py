...
def run_watcher(config_path: str, parallelism: int = 1):
    print(f"[watch] Starting watcher using config: {config_path}")
    ...
    if not watch_dirs:
        print("No input directories found to watch.")
        return

    print(f"[watch] Watching directories: {', '.join(watch_dirs)}")
    ...
    for path in watch_dirs:
        if os.path.exists(path):
            ...
            print(f"[watch] Warning: Watch path does not exist: {path}")
